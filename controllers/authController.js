...
module.exports = {
    ...
    loginUser: async (req, res) => { 
        ...
        !user && res.status.json({message: "Wrong credentials, provide a valid user email"});
        const decryptedPassword = CryptoJs.AES.decrypt(user.password, process.env.SECRET);
        const decryptedPass = decryptedPassword.toString(CryptoJs.enc.Utf8);

        decryptedPass !== req.body.password && res.status(401).json("Your password is incorrect");
